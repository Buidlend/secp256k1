...
export function bigintToBytes32(n: bigint): Uint8Array<ArrayBuffer> {
...
export async function rfc6979(privateKey: bigint, msgHash: bigint): Promise<bigint> {
  const bx = bigintToBytes32(privateKey)
  const bh = bigintToBytes32(msgHash)

  let V = new Uint8Array(32).fill(0x01) as Uint8Array<ArrayBuffer>
  let K = new Uint8Array(32).fill(0x00) as Uint8Array<ArrayBuffer>
...
function keccakF(S: Uint32Array): void {
  ...
  for (let round = 0; round < 24; round++) {
    // θ
    const g = (a: Uint32Array, i: number): number => a[i] ?? 0
    for (let x = 0; x < 5; x++) {
    ...
    for (let x = 0; x < 5; x++) {
      const xp = (x + 1) % 5; const xm = (x + 4) % 5
      const [dlo, dhi] = rotl64(C[2*xp]!, C[2*xp+1]!, 1)
      const dL = (dlo ^ C[2*xm]!) >>> 0; const dH = (dhi ^ C[2*xm+1]!) >>> 0
      for (let y = 0; y < 5; y++) {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      B[2*pi] = nlo; B[2*pi+1] = nhi
    }
    B[0] = S[0]!; B[1] = S[1]!
    for (let k = 0; k < 50; k++) S[k] = B[k]!
    // χ
    for (let y = 0; y < 5; y++) {
      const row = new Uint32Array(10)
      for (let x = 0; x < 5; x++) { row[2*x] = S[2*(x+5*y)]!; row[2*x+1] = S[2*(x+5*y)+1]! }
      for (let x = 0; x < 5; x++) {
...
export function keccakHash(input: Uint8Array): Uint8Array<ArrayBuffer> {
  const rate   = 136
  const padded = new Uint8Array(Math.ceil((input.length + 1) / rate) * rate) as Uint8Array<ArrayBuffer>
  padded.set(input)
  ...
  const output = new Uint8Array(32) as Uint8Array<ArrayBuffer>
  for (let i = 0; i < 4; i++) {
    const lo = state[2*i]!; const hi = state[2*i+1]!
    output[8*i+0] = lo & 0xff;        output[8*i+1] = (lo >> 8) & 0xff
    output[8*i+2] = (lo >> 16) & 0xff; output[8*i+3] = (lo >> 24) & 0xff
    output[8*i+4] = hi & 0xff;        output[8*i+5] = (hi >> 8) & 0xff
    output[8*i+6] = (hi >> 16) & 0xff; output[8*i+7] = (hi >> 24) & 0xff
  }
  return output
}

/** EIP-55 checksum address from 20-byte address */
export function toChecksumAddress(addr: Uint8Array): string {
