...
import { sha256, keccak256, bytes32ToBigint, bigintToBytes32, concat } from "./utils.js"
 ...
 * ```
 */
export async function send(recipientSpendPub: PublicKey): Promise<StealthAnnouncement> {
  const ephemeralKey = SecretKey.random()
  const ephemeralPub = ephemeralKey.toPublicKey()

  // Shared secret: ECDH then hash
  const sharedPoint  = recipientSpendPub.point.mul(ephemeralKey.scalar)
  const sharedSecret = keccak256(bigintToBytes32(sharedPoint.x))
  const s            = mod(bytes32ToBigint(sharedSecret), Curve.N)

  // Stealth public key
  const stealthPub = recipientSpendPub.tweak(s)

  return {
    ephemeralPubKey: ephemeralPub.toCompressed(),
    stealthAddress:  stealthPub.toEthAddress(),
  }
}
...
export async function receive(
  spendKey: SecretKey,
  ephemeralPubKey: Uint8Array,
): Promise<StealthRecovery> {
  const ephemeralPub = PublicKey.fromSec1(ephemeralPubKey)

  // Shared secret: ECDH then hash (mirrors sender)
  const sharedPoint  = ephemeralPub.point.mul(spendKey.scalar)
  const sharedSecret = keccak256(bigintToBytes32(sharedPoint.x))
  const s            = mod(bytes32ToBigint(sharedSecret), Curve.N)

  // Stealth private key = spend_key + s (mod N)
  const stealthKey = spendKey.tweak(s)
  const stealthPub = stealthKey.toPublicKey()

  return {
    stealthKey,
    stealthPub,
    address: stealthPub.toEthAddress(),
  }
}
...
export async function scan(
  spendKey: SecretKey,
  announcements: Array<{ ephemeralPubKey: Uint8Array; stealthAddress: string }>,
): Promise<Array<{ announcement: typeof announcements[0]; stealthKey: SecretKey; address: string }>> {
  const results = []
  for (const ann of announcements) {
    const recovery = await receive(spendKey, ann.ephemeralPubKey)
    if (recovery.address.toLowerCase() === ann.stealthAddress.toLowerCase()) {
      results.push({ announcement: ann, stealthKey: recovery.stealthKey, address: recovery.address })
    }
  }
  return results
}
 ...
 */
export function destroyKey(key: SecretKey): void {
  // JS doesn't allow true memory wiping, but we overwrite the object
  ;(key as unknown as { scalar: bigint }).scalar = 0n
}
