...
async function challengeHash(msgHash, ring, L, R) {
 ...
 * ```
 */
export async function sign(msgHash, signerKey, ring, signerIndex) {
    const n = ring.length;
    if (signerIndex < 0 || signerIndex >= n)
        throw new Error("Ring.sign: signerIndex out of range");
 ...
 * ```
 */
export async function verify(msgHash, sig) {
    try {
 ...
 */
export function isLinked(a, b) {
    return a.keyImage.equals(b.keyImage);
}
export function serialize(sig) {
    return {
