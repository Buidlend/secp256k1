 ...
 * ```
 */
export async function personalSign(key, message) {
 ...
 * ```
 */
export function personalRecover(message, sig) {
...
function encodeType(primaryType, types) {
    ...
    while (queue.length) {
        ...
        for (const field of (types[t] ?? [])) {
            ...
            if (types[base] && !visited.has(base))
                queue.push(base);
        }
    }
...
function abiEncode(type, value) {
    if (type === "address") {
        const hex = value.replace("0x", "").toLowerCase().padStart(64, "0");
        return hexToBytes(hex);
    }
    if (type === "bool")
        return bigintToBytes32(value ? 1n : 0n);
    if (type.startsWith("uint") || type.startsWith("int"))
        return bigintToBytes32(BigInt(value));
    if (type === "bytes32")
        return hexToBytes(value.replace("0x", "").padStart(64, "0"));
    if (type === "string")
        return keccak256(new TextEncoder().encode(value));
    if (type === "bytes")
        return keccak256(hexToBytes(value.replace("0x", "")));
...
function encodeData(primaryType, data, types) {
    ...
    const parts = fields.map(f => {
        ...
        if (types[baseType]) {
            return keccak256(encodeData(baseType, data[f.name], types));
        }
...
export function hashTypedData(params) {
    ...
    const domainFields = EIP712_DOMAIN_TYPE.filter(f => domain[f.name] !== undefined);
    const domainTypes = { EIP712Domain: domainFields };
    const domainSep = encodeData("EIP712Domain", domain, domainTypes);
 ...
 * ```
 */
export async function signTypedData(key, params) {
 ...
 * ```
 */
export function ecrecover(msgHash, sig) {
...
function rlpEncode(input) {
    if (input instanceof Uint8Array) {
        if (input.length === 1 && (input[0] ?? 0x80) < 0x80)
            return input;
...
function rlpLength(len, offset) {
    ...
    const bytes = bigintToBytes32(BigInt(len)).filter((_, i, a) => {
        let start = 0;
        while (start < a.length - 1 && (a[start] ?? 0) === 0)
            start++;
        return i >= start;
    });
    // strip leading zeros properly
    let s = 0;
    const b = bigintToBytes32(BigInt(len));
    while (s < b.length - 1 && b[s] === 0)
        s++;
    const lenBytes = b.slice(s);
    return concat(new Uint8Array([offset + 55 + lenBytes.length]), lenBytes);
}
function bigintToRlpBytes(n) {
    if (n === 0n)
 ...
 * ```
 */
export async function signTx(key, tx) {
